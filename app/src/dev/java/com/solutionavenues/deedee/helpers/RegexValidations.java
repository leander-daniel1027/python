...
public class RegexValidations {
    ...
    public static final String MANDATORY_FIELD = "Please fill Mandatory fields.";
    ...
    private static final String Phone_MSG = "Phone Number should be 8 digits";
    ...
    public static final String CONFIRM_PASSWORD_EMPTY_MSG = "Please enter confirm password";
    ...
    public static boolean isEmailAddress(EditText editText, boolean required, Context context) {
        return isValid(editText, EMAIL_REGEX, context.getString(R.string.EMAIL_MSG), context.getString(R.string.EMAIL_EMPTY_MSG), required);
    }
   ...
   /* public static boolean isValidPassword(EditText password) {
    ...
    public static boolean isValidPassword(EditText editText,
                                          boolean isConfrimPassword, Context context) {
        boolean status = true;
        if (editText.getText().toString().length() < PASSWORD_LENGTH) {
            status = false;
            if (editText.getText().toString().length() == 0) {
                if (!isConfrimPassword) {
                    editText.setError(context.getString(R.string.PASSWORD_EMPTY_MSG));
                }else{
                    editText.setError(context.getString(R.string.CONFIRM_PASSWORD_EMPTY_MSG));
                }
            } else if(editText.getText().toString().length() < 6){
                editText.setError(context.getString(R.string.PASSWORD_VALIDATION_MSG));
            }
        }
        return status;
    }
    ...
    public static boolean hasText(EditText editText, Context context,String msg) {
        String text = editText.getText().toString().trim();
        editText.setError(null);
    ...
    public static boolean isValidName(EditText editText, Context context) {
        ...
        if (TextUtils.isEmpty(text)) {
            editText.setError(context.getString(R.string.EMPTY_NAME_MSG));
            return false;
        }
        if (text.length() < 3 || text.length() > 20) {
            editText.setError(context.getString(R.string.INVALID_NAME_MSG));
            return false;
        }
    ...
    public static boolean isPhoneNumber(EditText editText, boolean required, Context context) {
        return isValid(editText, PHONE_REGEX, context.getString(R.string.PHONE_MSG), context.getString(R.string.PHONE_EMPTY_MSG), required);
    }
